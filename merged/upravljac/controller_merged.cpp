...
// MQTT Configuration
#define MQTT_SERVER_ADDRESS "172.20.10.2"
...
void process_discovery_message(const std::string& message) {
    ...
    if (it == connected_devices.end()) {
        ...
        connected_devices[uuid] = device_info;
    }
}

void send_confirmation(int sockfd, struct sockaddr_in client_addr) {
...
int main(int argc, char* argv[]) {
    ...
    std::cout << std::endl;
    std::cout << "Controller logic:" << std::endl;
    std::cout << "- Machine shutdown: ON if temp ≥38.5°C or ≤35°C or heart ≥105 or ≤45 bpm" << std::endl;
    std::cout << "- Emergency call: ON only if heart ≥105 or ≤45 bpm" << std::endl;
    ...
    while (true) {
        ...
        if (message.find("M-SEARCH") != std::string::npos) {
        ...
        } else if (message.find("NTS: ssdp:alive") != std::string::npos) {
            process_discovery_message(message);
            send_confirmation(sockfd, server_addr);
            print_connected(connected_devices);
        } else if (message.find("NTS: ssdp:byebye") != std::string::npos) {
            process_discovery_message(message);
            std::istringstream iss(message);
            std::string line;
            while (std::getline(iss, line)) {
                ...
                if (pos != std::string::npos) {
                    ...
                    std::cout << "Found UUID: " << id << "." << std::endl;
            ...
            connected_devices[id].name = root["name"].asString();
